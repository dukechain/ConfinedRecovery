 ...
 */
public class KMeansCheckpointingTaskmanagerKillTest {
	...
	public void testProgram(int jobManagerPort, final File coordinateDir) throws Exception {
		...
		DataSet<Tuple1<Integer>> data = env.generateSequence(1, 5).map(new MapFunction<Long, Tuple1<Integer>>() {
			@Override
			public Tuple1<Integer> map(Long value) throws Exception {
				// TODO Auto-generated method stub
				return new Tuple1<Integer>(value.intValue());
			}
		});
		...
		DataSet<Tuple1<Integer>> result = data.flatMap(new RichFlatMapFunction<Tuple1<Integer>, Tuple1<Integer>>() {
			...
			Collection<Tuple1<Integer>> test;
			
			public void open(Configuration parameters) throws IOException {
				System.out.println(getIterationRuntimeContext().getSuperstepNumber());
				if(getIterationRuntimeContext().getSuperstepNumber() == 2) {
					if (!markerCreated) {
				...
				test = getRuntimeContext().getBroadcastVariable("testBC");
			}

			@Override
			public void flatMap(Tuple1<Integer> value,
					Collector<Tuple1<Integer>> out) throws Exception {
				Iterator<Tuple1<Integer>> i = test.iterator();
				while(i.hasNext()) {
					out.collect(new Tuple1<Integer>(i.next().f0 + 100));
				}
			}
		}).withBroadcastSet(iteration, "testBC");
