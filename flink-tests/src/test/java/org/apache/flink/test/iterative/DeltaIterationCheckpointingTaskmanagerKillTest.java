 ...
 */
public class DeltaIterationCheckpointingTaskmanagerKillTest {
	...
	public void testTaskManagerProcessFailure() {
		...
		try {
			...
			// start a JobManager
			scala.Tuple2<String, Object> localAddress = new scala.Tuple2<String, Object>("localhost", jobManagerPort);
			...
			jmActorSystem = AkkaUtils.createActorSystem(jmConfig, new Some<scala.Tuple2<String, Object>>(localAddress));
	...
	public void testProgram(int jobManagerPort, final File coordinateDir) throws Exception {
		...
		DataSet<Tuple2<Long, Long>> newWorkset =  iteration.getWorkset().map(new RichMapFunction<Tuple2<Long, Long>, Tuple2<Long, Long>>() {
			
			private boolean markerCreated = false;
			
			public void open(Configuration parameters) throws IOException {
				System.out.println(getIterationRuntimeContext().getSuperstepNumber());
				if(getIterationRuntimeContext().getSuperstepNumber() == 3) {
					if (!markerCreated) {
		...
		iteration.closeWith(iteration.getWorkset(), newWorkset).print();
