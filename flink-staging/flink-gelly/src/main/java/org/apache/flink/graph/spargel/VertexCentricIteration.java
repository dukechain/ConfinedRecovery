	...
	implements CustomUnaryOperation<Vertex<VertexKey, VertexValue>, Vertex<VertexKey, VertexValue>>
{
	 ...
	 * @return The operator that represents this vertex-centric graph computation.
	 */
	public DataSet<Vertex<VertexKey, VertexValue>> createBulkResult() {
		if (this.initialVertices == null) {
			throw new IllegalStateException("The input data set has not been set.");
		}
...
//		messages.writeAsCsv("file:/c:/temp/1");
//		
//		DataSet<Tuple2<VertexKey, Message>> messages2 = messages.union(messages);
		...
		return iteration.closeWith(updates);
